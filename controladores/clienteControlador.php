	...
	class clienteControlador extends mainModel{
        ...
        public function paginador_cliente_controlador($pagina,$registros,$url,$busqueda){
            ...
            if(isset($pagina) && $pagina != ""){
                $id_cliente = mainModel::decryption($pagina);
            }
            else{
                $id_cliente = "";
            }
            ...
            if(isset($id_cliente) && $id_cliente != ""){
                $consulta="SELECT SQL_CALC_FOUND_ROWS * FROM cliente WHERE cliente_id = '$id_cliente'";
            }
            elseif(isset($busqueda) && $busqueda!=""){
				$consulta="SELECT SQL_CALC_FOUND_ROWS * FROM cliente WHERE (cliente_id LIKE '%$busqueda%' OR cliente_nombre LIKE '%$busqueda%' OR cliente_apellido LIKE '%$busqueda%' OR cliente_email LIKE '%$busqueda%' OR cliente_provincia LIKE '%$busqueda%' OR cliente_ciudad LIKE '%$busqueda%') ORDER BY cliente_nombre ASC LIMIT $inicio,$registros";
            }
            else{
				...
				foreach($datos as $rows){
					$tabla.='
						<tr class="text-center" >
                            ...
                            <td>'.$rows['cliente_direccion'].', '.$rows['cliente_ciudad'].', '.$rows['cliente_provincia'].'</td>
